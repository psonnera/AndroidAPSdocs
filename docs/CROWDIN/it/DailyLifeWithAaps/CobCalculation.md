...
Normalmente **AAPS** rileva l'assorbimento dei carboidrati tramite le deviazioni di **glicemia**. Poiché questo metodo calcola solo l'assorbimento minimo di carboidrati senza considerare le deviazioni di **glicemia**, potrebbe portare a valori COB errati. Nel caso in cui l'utente abbia inserito dei carboidrati ma **AAPS** non riesce a rilevare il loro assorbimento stimato tramite le deviazioni di **glicemia**, utilizzerà il metodo [min_5m_carbimpact](#Preferences-min_5m_carbimpact) per calcolare l'assorbimento invece (il cosiddetto 'fallback').
