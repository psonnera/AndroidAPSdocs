...
- L'apk __AAPSClient__ è una versione ridotta di __AAPS__ in grado di seguire qualcuno, effettuare __Cambi di Profilo__, impostare __Obiettivi Temporanei__ e inserire carboidrati. Ci sono 2 app: [AAPSClient e AAPSClient2 da scaricare](https://github.com/nightscout/AndroidAPS/releases/). AAPSClient dovrebbe essere usato se il caregiver desidera installare l'apk due volte sullo stesso telefono per seguire 2 persone diverse (es. due bambini con diabete di tipo 1 ciascuno con il proprio account Nightscout).
