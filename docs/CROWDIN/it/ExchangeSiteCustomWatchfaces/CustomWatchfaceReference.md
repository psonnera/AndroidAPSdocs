...
=======
- Un file immagine denominato CustomWatchface (può essere un file bitmap `CustomWatchface.jpg`, `CustomWatchface.png` o un vettoriale `CustomWatchface.svg`). Questo file è la piccola icona usata per selezionare il quadrante quando si clicca sul pulsante "Carica quadrante", ed è anche l'immagine visibile nel plugin Wear di AAPS.
...
- La dimensione dello zip deve essere il più piccola possibile (meno di circa 500 kb). Se il file è troppo grande, verrà bloccato e non trasmesso all'orologio.
>>>>>>> Stashed changes
...
Dopo il primo blocco con i metadata, si impostano alcuni parametri generali (vedi [Elenco dei parametri generali](#cwf-reference-list-of-general-parameters) sotto), che consentono di impostare i colori del grafico (Carboidrati, Bolo, valori glicemia...) e i colori predefiniti per i valori in range, iper o ipo (colori predefiniti del valore glicemia e delle frecce).
...
- Editor di testo: si consiglia NotePad++ (o equivalente), un semplice editor di testo con il vantaggio di mostrare il testo formattato con codice colore, rendendo più facile individuare gli errori. Qualsiasi editor di testo semplice va comunque bene, poiché lo scopo è modificare le informazioni json.
...
Nel file json ci sono diverse chiavi per specificare i colori: `"color"`, `"fontColor"` per le viste, ma anche `"highColor"`, `"midColor"`, `"lowColor"`, ... (vedi [Elenco dei parametri generali](#cwf-reference-list-of-general-parameters))
...
Per aggiungere un'immagine di sfondo dedicata, è sufficiente includere nel file zip un file denominato `Background.jpg` (senza modificare nient'altro, inviare il file zip all'orologio e verificare il risultato).
...
=======
Per quanto riguarda l'immagine del valore glicemia, non ci sono scelte: può essere solo nel livello di sfondo (altrimenti sarà davanti alla vista del grafico e il grafico non sarà visibile!). Quindi occorre mappare il valore glicemia sullo sfondo e poi ruotare l'immagine di sfondo in base al valore glicemia.
>>>>>>> Stashed changes
...
- Per modificare le informazioni di formattazione in base alle impostazioni o ai valori, si può applicare tutta la funzionalità dynData; le chiavi dedicate sono `"invalidTextValue"` (senza "informazioni di formattazione" perché il valore non è valido) e `"textvalue1"`, `"textvalue2"`... (e tanti valori quanti si vogliono gestire tra minData e maxData)
...
- Tutti i valori `BG` sono in unità mgdl; per usare la funzionalità di formattazione per mostrare i valori in unità mmol occorre gestire la conversione mgdl→mmol. In un blocco `dynData` o `dynPref`, la chiave usata per nominare il blocco che includerà `"minValue"` e `"maxValue"` per la conversione del valore deve essere denominata `"dynValue": { ...  }`. (vedi [Chiavi Dyn Data](#cwf-reference-dyndata-keys))
...
| Chiave                        | Valore predefinito e commento |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
| Chiave                                                                                 | Tipo di vista       | Dati collegati                                                                                                                        | Chiave DynData                                                                                                                                                                             |
| -------------------------------------------------------------------------------------- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| `"direction"`<br/>`"direction_Ext1"` *<br/>`"direction_Ext2"` *            | Image View          | Frecce di tendenza                                                                                                                           | direction                                                                                                                                                                               |
