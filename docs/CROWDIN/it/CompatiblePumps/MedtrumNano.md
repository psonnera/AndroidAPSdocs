...
Verrà richiesta la conferma della disattivazione della patch corrente. Se non si è pronti ad attivare una nuova patch, premere **Annulla** per tornare alla schermata principale. **Si noti che questa azione non è reversibile.** Una volta completata la disattivazione, è possibile premere **Avanti** per continuare il processo di attivazione di una nuova patch.
